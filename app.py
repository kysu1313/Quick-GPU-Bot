...
def newegg_process_test(number, url, settings, debug=False):
    PROCESS_X = multiprocessing.Process(
        target=newegg_run, args=(number, settings.newegg_urls[settings.SELECTED_NEWEGG_URL], settings, debug))
    PROCESS_X.start()
    PROCESS_X.join()
...
def run_all(one, number, settings, debug=False):
    ...
    PROCESS_Y = multiprocessing.Process(
        target=bestbuy_run, args=(number, settings.bestbuy_urls[settings.SELECTED_BESTBUY_URL], settings, debug))
...
if __name__ == "__main__":
    ...
    try:
        ...
        #newegg_run(number, settings.newegg_urls[settings.SELECTED_NEWEGG_URL], settings, True)
        #newegg_process_test(number, settings.newegg_urls[settings.SELECTED_NEWEGG_URL], settings, True)
